...
$(function () {
  ...
  if (isToc) {
    if (showPreview) {
      $(".PostMain").append(
        '<div id="mdx-toc-preview" mdui-drawer="' +
...
function addToc(titleList) {
  ...
  $("#left-drawer nav").before(
    '<div class="mdui-tab mdui-tab-full-width" id="mdx-toc-select"><a href="#" id="mdx-toc-menu" class="mdui-ripple"><i class="mdui-icon material-icons">&#xe241;</i><label>' +
      mdx_toc_i18n_1 +
      '</label></a><a href="#" id="mdx-toc-toc" class="mdui-ripple"><i class="mdui-icon material-icons">&#xe86d;</i><label>' +
      mdx_toc_i18n_2 +
      "</label></a></div>"
  );
  ...
  if (tocShown || firstCall) {
    ...
    if (howFar >= $("article").offset().top + $("article").height() - 80) {
      ...
      for (let i = 1; i < titleArr.length; i++) {
        if (
          howFar >=
          $("article *[data-mdxtoc='" + titleArr[i] + "']").offset().top - 80
        ) {
      ...
      if (howFar > $("article").offset().top - 140) {
        ...
        $("#" + titleArr[counter] + "-preview").addClass(
          "mdx-toc-preview-item-active"
        );
        if (showPreview) {
          $("#mdx-toc-preview").css(
            "transform",
            "translateY(-" + ((counter + 1) * 20 - 4) + "px)"
          );
        }
        if (item.length > 0) {
          ...
          if (topDist + 48 > window.innerHeight && tocShown) {
            $("#left-drawer")
              ...
              .animate(
                {
                  scrollTop:
                    document.getElementById("left-drawer").scrollTop +
                    (topDist + 48 - window.innerHeight) +
                    8,
                },
                200
              );
